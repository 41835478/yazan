            ...
            <div class="panel">
                <div class="panel-body">
                    <ul class="nav nav-tabs">
                        ...
                        <li style="display: inline-block;line-height:20px;float:right;">
                            <a class="btn btn-primary" href="{{route('category.create')}}">添加系列</a>
                        </li>
                    </ul>
                    <table id="datatables" class="table table-striped table-no-border">
                        ...
                        <tr>
                            <td>
                                <a target="_blank" href="{{route('category.show', ['category'=>$category->id])}}">
                                    {{$category->name}}
                                </a>
                            </td>
                                                      
                            <td>{{$category_status[$category->status]}}</td>                         
                            <td>{{substr($category->created_at, 0 ,10)}}</td>      
                            <td class="center">
                                <!-- <a class="btn btn-success" target="_blank"  href="{{route('category.show', ['category'=>$category->id])}}">
                                    <i class="icon-edit icon-white"></i> 查看
                                </a> -->
                                <a class="btn btn-warning" href="{{route('category.edit', ['category'=>$category->id])}}">
                                    <i class="icon-edit icon-white"></i> 编辑
                                </a>
                                <input type="hidden" name="category_id" value="{{$category->id}}">
                                
                                <span>
                                <form action="{{route('category.destroy', ['category'=>$category->id])}}" method="post" style="display: inherit;margin:0px;">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
