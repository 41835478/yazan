...
$(document).ready(function(){
	...
	//用户代理等级ajax
	$('#agents_total')[0].selectedIndex = 0; 
	//获得一级代理列表
	...
	//获得三级代理列表
	$('#agents_secend').change(function(){

		var agents_secend = $(this).val();
		var token        = $("input[name='_token']").val();
		var request_url  = $("input[name='agents_ajax_request_url']").val();
		$('#agents_thrid').hide();
		// alert(agents_frist);return false;
        ...
        $.ajax({
			...
			success: function(data){		
				if(data.status == 1){
					var content = '<option  value="0">--三级代理--</option>';
					$.each(data.data, function(index, value){
						content += '<option value="';
						content += value.id;
						content += '">';
					...
					// console.log($('#agents_frist'));
					$('#agents_thrid').empty();
					$('#agents_thrid').append(content);
					// console.log(content);
					$('#agents_frist').css('display', 'inline-block');
					$('#agents_secend').css('display', 'inline-block');
					$('#agents_thrid').css('display', 'inline-block');
				}else{
					alert(data.message);
					$('#agents_thrid').empty();
					$('#agents_thrid').append('<option  value="0">--三级代理--</option>');
					$('#agents_thrid').hide();
					return false;
				}
			},		
